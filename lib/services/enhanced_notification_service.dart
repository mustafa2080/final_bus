...
// import 'global_notification_monitor.dart'; // File not found - commented out
...
class EnhancedNotificationService {
  ...
  // final GlobalNotificationMonitor _globalMonitor = GlobalNotificationMonitor(); // Commented out - service not available
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      // await _globalMonitor.startMonitoring(); // Commented out - service not available
  ...
  Future<void> _queueForGlobalDelivery({
    ...
    try {
      ...
      // await _globalMonitor.queueGlobalNotification( // Commented out - service not available
      ...
      debugPrint('✅ Notification queued for global delivery to: $userId (global monitor disabled)');
    } catch (e) {
  ...
  /// إشعار ترحيبي محسن لولي الأمر الجديد - محدود الاستخدام
  /// يتم إرساله فقط عند التسجيل الجديد وليس عند كل تسجيل دخول
  Future<void> sendWelcomeNotificationToNewParent({
    ...
    try {
      ...
      final welcomeRecord = await _firestore
          .collection('welcome_records')
          .doc(parentId)
          .get();
      
      if (welcomeRecord.exists && welcomeRecord.data()?['isWelcomeSent'] == true) {
        debugPrint('⚠️ Welcome notification already sent to: $parentName');
        return;
      }
      ...
      debugPrint('✅ Welcome notification sent to new parent: $parentName');
    } catch (e) {
      debugPrint('❌ Error sending welcome notification to new parent: $e');
    }
  }

  /// إشعار ترحيبي سريع لولي الأمر الجديد - مع منع التكرار
  Future<void> sendQuickWelcomeNotification({
    ...
    try {
      ...
      if (welcomeRecord.exists && welcomeRecord.data()?['isWelcomeSent'] == true) {
        debugPrint('⚠️ Quick welcome notification already sent to: $parentName');
        return;
      }
  ...
  /// إرسال إشعار عام لجميع الإداريين
  Future<void> sendGlobalAdminNotification({
    required String title,
    ...
    try {
      debugPrint('📢 Sending global admin notification: $title');
      ...
      debugPrint('✅ Global admin notification sent successfully');
    } catch (e) {
      debugPrint('❌ Error sending global admin notification: $e');
    }
  }

  /// تنظيف الموارد وإزالة الإشعارات المكررة
  void dispose() {
    debugPrint('🧹 Disposing EnhancedNotificationService...');
    // تنظيف الإشعارات المكررة من الطابور العالمي
  ...
  Future<void> _cleanupDuplicateNotifications() async {
    try {
      debugPrint('🧹 Cleaning up duplicate welcome notifications...');
      
      // حذف الإشعارات المتعددة من fcm_global_queue
      final duplicateQueue = await _firestore
          .collection('fcm_global_queue')
          .where('payload.notification.title', isEqualTo: '🎉 أهلاً وسهلاً بك في MyBus')
          .where('status', isEqualTo: 'pending')
          .get();
      
      final batch = _firestore.batch();
      for (var doc in duplicateQueue.docs) {
        batch.delete(doc.reference);
      }
      
      if (duplicateQueue.docs.isNotEmpty) {
        await batch.commit();
        debugPrint('✅ Cleaned up ${duplicateQueue.docs.length} duplicate notifications from global queue');
      }
    } catch (e) {
